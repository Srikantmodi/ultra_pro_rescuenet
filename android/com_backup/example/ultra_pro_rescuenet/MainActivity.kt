...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        val generalHandler = GeneralHandler(this, discoveryHandler, socketHandler, connectionHandler)
            ...
            override fun onReceive(context: Context, intent: Intent) {
                when(intent.action) {
                    ...
                    WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION -> {
                        Log.d("RescueNet", "⚡ Peers Changed Intent Received")
                        
                        // FIX: Request peers immediately when the intent fires
                        manager.requestPeers(channel) { peers ->
                            Log.d("RescueNet", "Physical Device List: ${peers.deviceList.size} devices found")
                            for (device in peers.deviceList) {
                                Log.d("RescueNet", " -> Found: ${device.deviceName} (${device.deviceAddress})")
                            }
                            
                            // Optional: Pass these raw peers to your discoveryHandler if you want a fallback list
                            // discoveryHandler.updateRawPeers(peers.deviceList) 
                        }
                    }
