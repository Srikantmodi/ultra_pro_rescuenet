...
class ConnectionHandler(
    ...
    fun getGroupOwnerIp(): String? {
        return currentConnectionInfo?.groupOwnerAddress?.hostAddress
    }
    ...
    // NEW: Public connect for GeneralHandler
    @SuppressLint("MissingPermission")
    fun connect(address: String, callback: (Boolean, String?) -> Unit) {
        ...
        manager.connect(channel, config, object : WifiP2pManager.ActionListener {
            override fun onSuccess() {
                Log.d(TAG, "Connection init success")
                isConnecting = false
                Handler(Looper.getMainLooper()).postDelayed({
                    requestConnectionInfoInternal { info ->
                        callback(true, null)
            ...
            override fun onFailure(code: Int) {
                isConnecting = false
                callback(false, "Connect failed: ${getErrorMessage(code)}")
            }
        })
    }
